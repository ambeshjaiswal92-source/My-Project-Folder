      ...
      <div className="row g-4">
        ...
        <div className="col-12">
          <div className="card-dark">
            <div className="d-flex justify-content-between align-items-center mb-4">
              <h5 className="text-white mb-0">
                <i className="bi bi-images me-2 text-warning"></i>Homepage Slider Images
              </h5>
              <button 
                className="btn btn-primary btn-sm"
                onClick={() => setShowAddSlider(true)}
              >
                <i className="bi bi-plus-lg me-2"></i>Add Slider
              </button>
            </div>

            <p className="text-muted-custom mb-4">
              Manage the hero slider images displayed on the homepage. Drag to reorder, toggle to enable/disable.
            </p>

            {/* Slider Grid */}
            <div className="row g-3">
              {sliderImages.map((slider, index) => (
                <div key={slider.id} className="col-lg-4 col-md-6">
                  <div className={`card bg-dark border ${slider.active ? 'border-success' : 'border-secondary'} h-100`}>
                    <div className="position-relative">
                      <img 
                        src={slider.image} 
                        alt={slider.title}
                        className="card-img-top"
                        style={{ height: '150px', objectFit: 'cover', opacity: slider.active ? 1 : 0.5 }}
                        onError={(e) => {
                          e.target.src = 'https://placehold.co/400x200/1a1a2e/ffffff?text=Image+Not+Found'
                        }}
                      />
                    ...
                    <div className="card-body p-3">
                      <h6 className="text-white mb-1">{slider.title}</h6>
                      <p className="text-muted-custom small mb-2">{slider.subtitle || 'No subtitle'}</p>
                      {slider.offer && (
                        <div className="mb-2">
                          <span className="badge bg-danger small">
                            <i className="bi bi-tag-fill me-1"></i>{slider.offer}
                          </span>
                        </div>
                      )}
                      ...
                      <div className="d-flex gap-2">
                        <button 
                          className={`btn btn-sm flex-grow-1 ${slider.active ? 'btn-outline-warning' : 'btn-outline-success'}`}
                          onClick={() => toggleSliderActive(slider.id)}
                        >
                          <i className={`bi ${slider.active ? 'bi-eye-slash' : 'bi-eye'} me-1`}></i>
                          {slider.active ? 'Hide' : 'Show'}
                        </button>
                        <button 
              ...
              {sliderImages.length === 0 && (
                <div className="col-12">
                  <div className="text-center py-5">
                    <i className="bi bi-images display-4 text-muted-custom mb-3"></i>
                    <p className="text-muted-custom">No slider images. Click "Add Slider" to add your first image.</p>
                  </div>
                </div>
              )}
            </div>

            <div className="mt-4 p-3 rounded" style={{ background: 'rgba(255, 133, 51, 0.1)' }}>
              <small className="text-muted-custom">
                <i className="bi bi-lightbulb me-2 text-warning"></i>
                <strong>Tip:</strong> Use high-quality images (1200×500px recommended) for best results. Only active sliders will be displayed on the homepage.
              </small>
            </div>
          </div>
        </div>
